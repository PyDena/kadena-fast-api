...
async def get_current_cut() -> response_models.Cut:
    ...
    hashes = {
        k: response_models.ChainCut(**v) for k, v in response_json.get("hashes").items()
    }
...
@app.get("/header/{chain}/{min_height}/{max_height}")
async def get_headers(
    chain: int, min_height: int, max_height: int
) -> response_models.Header:
    header_url = f"https://estats.chainweb.com/chainweb/0.0/mainnet01/chain/{chain}/header?minheight={min_height}&limit={max_height}"
    ...
    response_json: dict | list = response.json()
    header_items = [
        response_models.HeaderItem(**item) for item in response_json.get("items")
    ]
...
@app.get("/payload/{chain}/{payloadHash}")
async def get_recent_transactions(
    chain: int, payloadHash: str
) -> response_models.Payload:
    payload_url = f"https://estats.chainweb.com/chainweb/0.0/mainnet01/chain/{chain}/payload/{payloadHash}/outputs"
    response = requests.get(payload_url)
    response_json = response.json()
    return response_models.Payload(**response_json)


@app.get("/code/{searchTerm}")
async def get_recent_code_executions(
    searchTerm: str,
) -> list[response_models.TransactionCode]:
    code_url = (
        f"https://estats.chainweb.com/txs/search?search={searchTerm}&offset=0&limit=20"
    )
    response = requests.get(code_url)
    response_json = response.json()
    return [response_models.TransactionCode(**tx) for tx in response_json]
